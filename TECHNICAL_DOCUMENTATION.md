...
**Repository:** manasvii03/Blogging-Website-Mentorship  
...
class Blog(models.Model):
    ...
    # Interaction Fields (Many-to-Many)
    ...
    tags               → ManyToManyField(Tag)           # Linked Tags
    
    # Computed Properties
    read_time          → @property (auto-calculated)    # Returns: int (minutes)
...
- **Auto-timestamps:** created_at and updated_at for sorting and filtering
- **Read Time Calculation:** formula: word_count / 50 (approximates 200 wpm reading speed), returns: minimum 1 minute
...
- Frontend sends tags as a **list of strings**: ["tech", "coding", "python"]
- Backend auto-creates tags if they don't exist
- Enables content discovery and filtering

---

### 3.3 User Model (Django Built-in)
...
### 3.4 Favourites Model (Independent Collection)
...
- Allows users to explicitly save blogs to a favorites list
...
**Key Settings from backend/settings.py:**
...
**Query Parameters:**
- search=<string> - Search in: title, content, category, tags (case-insensitive)
...
**Example Flow:**
```
1st POST → { "status": "liked", "like_count": 13 }
2nd POST → { "status": "unliked", "like_count": 12 }
3rd POST → { "status": "liked", "like_count": 13 }
```
...
| List All | GET /api/blogs/ | ✓ Paginated, sortable by date |
| Search | GET /api/blogs/?search=X | ✓ Searches 4 fields (title, content, category, tags) |
...
| Star (Favorite) | POST /api/blogs/{id}/star/ | Toggle on/off | Not counted in response |
| Bookmark | POST /api/blogs/{id}/bookmark/ | Toggle on/off | Not counted in response |

### 7.3 Collections & Curation

| Feature | Endpoint | Storage |
|---------|----------|---------|
| Liked Blogs | GET /likedblogs/ | ManyToMany relation on Blog |
| Bookmarks | GET /bookmarkedblogs/ | ManyToMany relation on Blog |
| Favourites | POST/GET /favourite/ & /favblogs/ | Separate Favourites model |

### 7.4 Smart Features

| Feature | Auto-Calculated | Usage |
|---------|-----------------|-------|
| Read Time | Yes (word count / 50) | Displayed on blog cards |
| Timestamps | Yes (auto_now_add/auto_now) | Sort and filter |
| Tag System | Dynamic creation | Content discovery |
| Media Serving | Configured in settings | Images uploaded to /media/ |
...
- **Pagination:** Add limit/offset to GET /blogs/ (currently returns all)
...
Potential issue: Getting liked_blogs with .all() might hit database multiple times. Solution:
```python
...
class BlogSerializer(serializers.ModelSerializer):
    read_time = serializers.ReadOnlyField()  # Auto-calculated
...
**Document Complete** - Ready for Interview Preparation